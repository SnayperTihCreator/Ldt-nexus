...
class LdtSettings:
    ...
    @classmethod
    def serializer(cls, objType: Type[SerializableProtocol]):
    ...
    def _unpack(self, data: Any) -> Any:
        ...
        if "_tag" in data:
            class_name = data.pop("_tag")
            
            if class_name in self._decoders:
                return self._decoders[class_name](data)
            
            raise ValueError(f"Found tag but no decoder registered for {class_name}")
        
        return {k: self._unpack(v) for k, v in data.items()}
    
    def __attrs_post_init__(self):
        self._core = _ldt_core.LdtSettingEngine(self.path, self.driver)
    
    def setValue(self, key: str, value: Any):
        packed = self._pack(value)
        self._core.set_value(key, packed)
    
    def value(self, key: str, default: Any = None) -> Any:
        raw = self._core.value(key, default)
    ...
    @contextmanager
    def localGroups(self, *names: list[str]):
        for name in names:
            self.beginGroup(name)
        try:
            yield self
